...
- **Carrousel photo/vidéo en haut de l'accueil, géré depuis l'admin** : photos (upload direct ou lien Drive) et vidéos (lien Drive, YouTube, ou URL directe) défilent automatiquement en fond du hero. Tant qu'aucun média n'est ajouté, le hero garde son fond d'origine (rien à faire, dégradation silencieuse)
...
- **Catalogue de formules géré depuis l'admin** (comme la galerie) : titre, description, image et liste d'options par formule, chaque option révélée au clic — sans prix affichés, devis personnalisé systématique. Une formule « Photobooth » s'affiche par défaut tant qu'aucune n'a été ajoutée depuis l'admin. Les formules du catalogue peuvent aussi être **modifiées** après coup (pas seulement ajoutées/supprimées)
...
- **Flyer par option de catalogue** : chaque option d'une formule peut avoir son propre flyer explicatif (PDF ou image, uploadé depuis l'admin ou lien Drive) — quand un visiteur choisit cette option précise, le flyer s'ouvre automatiquement dans un nouvel onglet
...
- **Page d'administration** (`admin.html`, protégée par mot de passe, elle aussi en onglets) : bandeau de statistiques (demandes du mois, évènements confirmés à venir, acomptes encaissés, taux de conversion devis→payé), réservations avec suivi de statut, badges client récurrent/réduction en attente et gestion de l'acompte, activation de l'accès aux photos, gestion de la galerie, du catalogue et du carrousel de l'accueil, envoi de newsletter, export CSV, **modification des coordonnées de contact du site** (téléphone, email affiché, WhatsApp, zone géographique, horaires) et **réseaux sociaux en liste libre** (Instagram, TikTok, LinkedIn, site web... autant que voulu, pas limité à une liste figée) — sans jamais ouvrir le Google Sheet
